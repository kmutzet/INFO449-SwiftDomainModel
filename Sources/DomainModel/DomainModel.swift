...
public struct Money {
    ...
    func convert(_ targetCurrency: String) -> Money {
        let targetCurrencyUp = targetCurrency.uppercased()
        guard ["USD", "GBP", "EUR", "CAN"].contains(targetCurrencyUp) else {
            print("Unknown currency")
            return Money(amount: 0, currency: targetCurrencyUp)
        }
        let conversionRates: [String: Double] = ["USD": 1.0, "GBP": 0.5, "EUR": 1.5, "CAN": 1.25]
        let amountUSD = Double(amount) / conversionRates[currency]!
        let convertedAmount = Int(amountUSD * conversionRates[targetCurrencyUp]!)
        return Money(amount: convertedAmount, currency: targetCurrencyUp)
    }
...
public class Family {
    ...
    func householdIncome(_ hoursWorked: Int? = nil) -> Int {
        if let hoursWorked = hoursWorked {
            ...
            return members.compactMap { $0.job?.calculateIncome(2000) }.reduce(0, +) // Assuming 2000 hours worked annually
        }
    }
